...
(defun rails-controller-layout:keymap (type)
  (let* ((name (capitalize (substring (symbol-name type) 1)))
    ...
    (when type
      ...
      (define-keys map
        ((rails-key "ci") 'rails-controller-layout:switch-to-migration)
        ((rails-key "cm") 'rails-controller-layout:switch-to-model)
        ((rails-key "ch") 'rails-controller-layout:switch-to-helper)
        ((rails-key "cf") 'rails-controller-layout:switch-to-functional-test)
        ((rails-key "cc") 'rails-controller-layout:switch-to-controller)
        ((rails-key "cu") 'rails-controller-layout:switch-to-unit-test)
